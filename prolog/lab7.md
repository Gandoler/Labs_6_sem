...
Этот предикат преобразует арифметическое выражение, включающее операции +, *, ^, в нормальную форму многочлена.

polynomize/2 вызывает simplify/2, который рекурсивно упрощает выражение.
simplify/2:
Раскрывает сложение и умножение.
Обрабатывает возведение в степень.
combine/3 упрощает сложение чисел.
distribute/3 упрощает умножение чисел.
expand_power/3 разворачивает A^N в A * A * ... * A.
