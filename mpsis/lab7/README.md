...
```
адресс памяти = 3 тк это ps/2, по неме прочитали 1c (a- по AT кодировке, что равно 61(hex) в ASCII)

```asm
28: 00038403        lb x8, 0(x7)                # берем данные из таблицы подстановки
```

по адрессу 0+1с = 1с берем значение в ascii-61 -------------- 270.215

```asm
2c: 00810023        sb x8, 0(x2)                # загружаем ascii-значение в vga
```
 по аджрессу 7 сохраняем значение из x8 = 61 (аски код)
