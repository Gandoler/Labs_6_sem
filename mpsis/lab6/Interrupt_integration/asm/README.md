...
|---------------|--------------------------------|------------------------------------------------------------------------------------|------------|-----------------------------------|
| 0x000007b7    | lui x15, 0x0                  | Загрузка верхних 20 бит (0x00000) в x15                                           | 10-50      | x15 = 0x29000                     |
| 0x00578793    | addi x15, x15, 0x5             | Добавление 0x5 к x15 (x15 = 0x005)                                              | 50-70      | x15 = 0x29005                                  |
| 0x00f02023    | sw x15, 0(x0)                  | Сохранение значения x15 (0x005) по адресу 0x0                                   | 70-110     | сохранение                                   |
| 0x000290b7    | lui x1, 0x29                | Загрузка верхних 20 бит (0x29000) в x1                                            | 110-130    | x1 = 0x00000                      |
| 0x00508093    | addi x1, x1, 0x5               | Добавление 0x5 к x1 (x1 = 0x29005)                                                | 130-150    | x1 = 0x00005                                   |
...
|##############|
|##############|
| 0x0060f3b3    | and x7, x1, x6                 | x7 = x1 & 0x1 (младший бит x1)                                                    |   |    вырезаем младший бит     |
| 0x004393b3    | sll x7, x7, x4                 | x7 = x7 << x4 (сдвиг на значение счетчика)                                        |   |   сдвигаем этот бит на его позицию           |
| 0x00746433    | or x8, x8, x7                  | x8 = x8 | x7 (накопление результата)    тут надо как раз смотреть                 |     |      добавляем его в число ответа      |
| 0x00620233    | add x4, x4, x6                 | x4 = x4 + 1 (инкремент счетчика)                                                  |     |  увеличиваем счетчик позиции      |
| 0x0010d093    | srli x1, x1, 1                 | x1 = x1 >> 1 (сдвиг вправо на 1 бит)                                              |    |   сдвигаем число на удаленнйы бит       |
| 0xfea090e3    | bne x1, x10, -32               | Переход на loop_start если x1 != 0                                                 |    |  если число еще не ноль возврат       |
| 0x00140413    | add x8, x8, x0                     | для вывода ответа                                                                |  |                                   |
| 0x0000006f    | jal x0, -4                      | Бесконечный цикл                                                                   |     |                                   |
|##############|
|##############|
| 0x0030d093    | srli x1, x1, 3                 | x1 = x1 >> 3 (сдвиг вправо на 3 бита)                                             |   | сдвигаем число на удаленнйы 3 бита  |
| 0xfca09ae3    | bne x1, x10, -44               | Переход на loop_start если x1 != 0                                                 |   |  если число еще не ноль возврат     |
| 0xff5ff06f    | jal x0, -12                    | Переход назад на 12 байт                                                           |    |                                   |
...
| 0x00300493    | addi x9, x0, 3                 | Инициализация x9 = 3 (константа)                                                  | 310-330    | x9 = 3                                 |
| 0x00000513    | addi x10, x0, 0                | Инициализация x10 = 0 (константа для сравнения)                                   | 330-350    | x10 = 0                                  |
| 0x00317133    | and x2, x2, x3                 | Применение маски 0x7 к x2 (x2 = x2 & 0x7)                                         | 350-370    | тут мы получаем три правых бита   sw_i =101  |
| 0x0030f2b3    | and x5, x1, x3                 | Применение маски 0x7 к x1 (x5 = x1 & 0x7)                                         | 370-390    |тут мы получаем три правых бита   a =101|
| 0x02510063    | beq x2, x5, 32                 | Переход на match_case если x2 == x5                                                | 390-410    | они равны поэтому         
|##############|
|##############|
| 0x0090d0b3    | srl x1, x1, x9                 | x1 = x1 >> 3 (сдвиг вправо на 3 бита)                                              | 410-430   | сдвигаем число на удаленнйы 3 бита = 5200  |
| 0xfca098e3    | bne x1, x10, -44               | Переход на loop_start если x1 != 0                                                 | 430-450   |  если число еще не ноль возврат  прыгаем на   0x00317133 |
|##############|
|##############|
| 0x00317133    | and x2, x2, x3                 | Применение маски 0x7 к x2 (x2 = x2 & 0x7)                                         | 450-470    | тут мы получаем три правых бита   sw_i =101  |
| 0x0030f2b3    | and x5, x1, x3                 | Применение маски 0x7 к x1 (x5 = x1 & 0x7)                                         | 470-490    |тут мы получаем три правых бита   a =000|
| 0x02510063    | beq x2, x5, 32                 | Переход на match_case если x2 == x5                                                | 490-510    | они не равны поэтому    
|##############|
|##############|
| 0x0060f3b3    | and x7, x1, x6                 | x7 = x1 & x6 (младший бит x1)                                                    |510-530   |    вырезаем младший бит   это 0   |
| 0x004393b3    | sll x7, x7, x4                 | x7 = x7 << x4 (сдвиг на значение счетчика)                                        |530-550   |   сдвигаем этот бит на его позицию  также ноль          |
| 0x00746433    | or x8, x8, x7                  | x8 = x8 | x7 (накопление результата)    тут надо как раз смотреть                 | 550-570    |      добавляем его в число ответа      |
| 0x00620233    | add x4, x4, x6                 | x4 = x4 + 1 (инкремент счетчика)                                                  | 570-590    |  увеличиваем счетчик позиции      станет 1 |
| 0x0010d093    | srli x1, x1, 1                 | x1 = x1 >> 1 (сдвиг вправо на 1 бит)                                              | 590-610   |   сдвигаем число на удаленнйы бит  а будет  2900  |
| 0xfea090e3    | bne x1, x10, -32               | Переход на loop_start если x1 != 0                                                 |610-630    |  если число еще не ноль возврат       |
|##############|
|так еще 8 раз а а значит на |
|##############|                        
| 0x0060f3b3    | and x7, x1, x6                 | x7 = x1 & x6 (младший бит x1)                                                    |510-530   |    вырезаем младший бит   это 0   |
| 0x004393b3    | sll x7, x7, x4                 | x7 = x7 << x4 (сдвиг на значение счетчика)                                        |530-550   |   сдвигаем этот бит на его позицию  также ноль          |
| 0x00746433    | or x8, x8, x7                  | x8 = x8 | x7 (накопление результата)    тут надо как раз смотреть                 | 550-570    |      добавляем его в число ответа      |
...
туда сюда и все рабоатет
![image](https://github.com/user-attachments/assets/dcbdd534-e863-4afc-abdc-4ce12b5d9c21)
